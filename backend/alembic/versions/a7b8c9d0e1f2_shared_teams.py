"""shared teams: user + userteam membership, join codes

Replaces per-row owner_id with a users table and a membership join, so a team
(and its matches/roster) can be shared across users. Existing data is migrated:
each team's current owner_id becomes a user + a membership, and every team gets
a unique join code. The owner_id columns are then dropped.

NOTE: this revision creates `user` / `userteam`; a later revision renames them to
`users` / `user_team` and adds a serial id (do not edit this file in place —
